...
class winbind::cnhrs (
  $domainadminuser 	= 'prometeu',
  $domainadminpw	= '@V5791alto',
  $realm		= 'cnhgo.local',
  $domain		= 'CNHGO',
#  $server		= 'ACADEMOS',
  ...
  file { 'smb.conf':
    ...
    owner   => 'root',
    group   => 'root',
    content => template('winbind/smb.conf_go.erb'),
...
#    command => "/usr/bin/net join -w ${domain} -S ${server} -U ${domainadminuser}%${domainadminpw}",
    command => "/usr/bin/net join -w ${domain} -U ${domainadminuser}%${domainadminpw}",
...
file { 'krb5.conf':
         ...
         group  => 'root',
         mode   => '0644',
         source => "puppet:///modules/winbind/krb5_go.conf",
     }
